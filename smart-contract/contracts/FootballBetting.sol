...
contract FootballBetting {
    ...
    function bet(uint256 matchId, uint8 team) external payable matchExists(matchId) {
        ...
        players[matchId].push(msg.sender);
    ...
    function claimReward(uint256 matchId) external matchExists(matchId) {
        ...
        if (players[matchId].length == 1 && players[matchId][0] == msg.sender) {
            _refund(matchId, b, msg.sender);
            return;
        }
    ...
    function _refund(uint256 matchId, Bet storage b, address user) internal {
        b.claimed = true;
        uint256 refund = b.amount;
        b.amount = 0;
        (bool sent, ) = user.call{value: refund}("");
        require(sent, "Failed to refund");
        emit RewardClaimed(matchId, user, refund);
    }

    function _noWinner(Match storage m) internal view returns (bool) {
        if (m.result == 1 && m.totalBetA == 0) return true;
        if (m.result == 2 && m.totalBetB == 0) return true;
        if (m.result == 3 && m.totalBetDraw == 0) return true;
        return false;
    }
