...
export default function UserPanel({ provider, address }) {
  ...
  async function fetchMatches() {
    try {
      ...
      for (let i = 0; i < matchCount; i++) {
        ...
        let playerCount = 0;
        try {
          playerCount = Number(await contract.getPlayerCount(i));
        } catch (e) {
          playerCount = 0;
        }
        arr.push({
  ...
  function canRefund(match, myBets) {
    if (!myBets[match.id] || myBets[match.id].claimed) return false;
    // Single player
    if (match.playerCount === 1) return true;
    // No one bet on the correct result
    if (
      (match.result == 1 && Number(match.totalBetA) === 0) ||
      (match.result == 2 && Number(match.totalBetB) === 0) ||
      (match.result == 3 && Number(match.totalBetDraw) === 0)
    ) {
      return true;
    }
    return false;
              ...
              <td style={{padding: '10px 16px'}}>
                {!m.finished ? (
                  ...
                  !myBets[m.id] || String(myBets[m.id].amount) === "0"
                    ...
                    : canRefund(m, myBets)
                      ? <button style={{background: '#ffb300', color: '#222', border: 'none', borderRadius: 8, padding: '6px 16px', fontWeight: 600, cursor: 'pointer'}} onClick={() => claimReward(m.id)}>Claim Refund</button>
